...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        logger.add_scalar('train_accuracy', sum(metrics['train_acc']) / len(metrics['train_acc']), global_step - 1)
        ...
        logger.add_scalar('val_accuracy', sum(metrics['val_acc']) / len(metrics['val_acc']), global_step - 1)
